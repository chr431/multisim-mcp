...
class Calibration:
    """Mapping between raster pixels and Multisim drawing units (mils).

    ``analysis_size_px`` is the size of the array the analysis actually ran on,
    which may be smaller than ``source_size_px`` when a downscale was applied.
    Any pixel coordinate reported alongside this calibration is in *analysis*
    pixels; :meth:`analysis_to_source` converts one for drawing on the original.
    """
...
def paper_candidates(
    ...
    *,
    ...
    implied ``px_per_mil``, so a caller can compare candidates and pick one.  A
        ...
        out.append(
            {
                ...
                "px_per_mil": round(width_px / (oriented[0] * MIL_PER_MM), 6),
                "dpi": round(width_px / (oriented[0] / 25.4), 2),
            }
        )
    out.sort(key=lambda item: -item["px_per_mil"])
    return out
...
def calibrate_from_dpi(
    ...
    *,
    ...
    This is the most reliable route when it is known.  A raster export's scale is
    ``dpi / 1000`` drawing units per pixel, independent of which paper size was
    used, so it sidesteps the A-series ambiguity entirely.  The page size is then
    whatever the pixel dimensions imply, which is exactly right for reproduction:
    the drawing occupies the same number of mils it did in the original editor.
    """
    numpy = _require_numpy()
    ...
    px_per_mil = dpi / 1000.0
    page_mm = (width_px / px_per_mil * 0.0254, height_px / px_per_mil * 0.0254)
    return Calibration(
        px_per_mil=px_per_mil,
        page_mm=page_mm,
        paper="Custom",
...
def calibrate(
    ...
    *,
    ...
    """Recover the pixel-to-mil scale of ``image``.

    Priority: explicit ``dpi``, then ``page_width_mm``, then a named ``paper``,
    then automatic detection.

    ``dpi`` is preferred because it is the only route that is unambiguous: every
    ISO A-series sheet shares one aspect ratio, so a drawing's scale cannot be
    recovered from its shape, and a wrong paper choice mis-scales every
    coordinate.  When detection is ambiguous this raises rather than guessing.
    """
    if dpi is not None:
    ...
    px_per_mil = width_px / (size[0] * MIL_PER_MM)
    if not 0.001 < px_per_mil < 1000.0:
        raise RasterError(
            f"implausible calibration {px_per_mil:.6f} px/mil for a "
